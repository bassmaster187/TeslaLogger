...
namespace TeslaLogger
{
    class MapQuest
    {
        ...
        public static void CreateMap(int startpos, int endpos, int Carid)
        {
                ...
                using (MySqlConnection con = new MySqlConnection(DBHelper.DBConnectionstring))
                {
                    ...
                    using (MySqlCommand cmd = new MySqlCommand("SELECT startposid, endposid, carid FROM teslalogger.trip order by startdate desc ", con))
                    {
                        ...
                        try
                        {
                            while (dr.Read())
                            {
                                CreateMap(Convert.ToInt32(dr["startposid"]), Convert.ToInt32(dr["endposid"]), Convert.ToInt32(dr["carid"]));
                            }
                        }
                        catch (Exception ex)
                        {
        ...
        public static void createAllChargigMaps()
        {
            ...
            try
            {
                ...
                using (MySqlConnection con = new MySqlConnection(DBHelper.DBConnectionstring))
                {
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(@"SELECT avg(lat) as lat, avg(lng) as lng, replace(replace(replace(replace(replace(convert(address USING ascii), '?',''),' ',''),'/',''),'&',''),',','') as name 
